...
export interface VueDatePickerProps {
  ...
  multiCalendars?:
    | DpOptionEnabled
    | Partial<{ static: boolean; solo: boolean; count: number | string }>;
  ...
  weekNumbers?:
    | WeekNumberType
    | { type: WeekNumberType; hideOnOffsetDates?: boolean };
...
// Explicitly export slots to avoid compilation errors
...
interface SidebarSlotProps {
  ...
  updateMonthYear?: (
    instance: number,
    val: { month: number; year: number; fromNav?: boolean },
  ) => void;
